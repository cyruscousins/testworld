...
public class MappedSVF2D extends SVF2D{
	...
	public MappedSVF2D(float[] data, int length, float mapSize){
		this.data = data;
		this.length = length;
		l1 = length + 1;
		this.mapSize = length / mapSize;
	}
	public static MappedSVF2D translateToSVF(float[] data, int length, float scale){
		int l1 = length + 1;
		...
		return new MappedSVF2D(ndata, length, scale);
	}
	public static MappedSVF2D translateToSVF(float[][] data, int length, float scale){
		int l1 = length + 1;
		float[] ndata = new float[l1 * l1];
